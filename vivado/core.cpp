...
void decode(const struct FtoDC ftoDC, struct DCtoEx& dctoEx, const ac_int<32, true> registerFile[32], const ac_int<32, true> registerFileCopy[32])
{
  ...
  const ac_int<32, false> valueReg2 = registerFileCopy[rs2];
...
//             bool globalStall)
void doCycle(struct Core& core, // Core containing all values
             bool globalStall,unsigned int  dataI [1<<24],unsigned int  dataM [1<<24] , volatile unsigned int * out1  )
{

#pragma HLS RESOURCE variable=core.regFile core=RAM_2P_URAM:
#pragma HLS RESOURCE variable=core.regFileCopy core=RAM_2P_URAM:
#pragma HLS DEPENDENCE variable=core.regFile inter WAR false
#pragma HLS DEPENDENCE variable=core.regFileCopy inter WAR false
  // printf("PC : %x\n", core.pc);
  ...
  core.im.process(dataI,core.pc, WORD, (!localStall && !core.stallDm) ? LOAD : NONE, 0, nextInst, core.stallIm);

  fetch(core.pc, ftoDC_temp, nextInst);
  decode(core.ftoDC, dctoEx_temp, core.regFile, core.regFileCopy);
  ...
  //core.dm->process(memtoWB_temp.address, mask, opType, memtoWB_temp.valueToWrite, memtoWB_temp.result, core.stallDm);
   core.dm.process(dataM,memtoWB_temp.address, mask, opType, memtoWB_temp.valueToWrite, memtoWB_temp.result, core.stallDm,out1 );
  ...
  if (wbOut_temp.we && wbOut_temp.useRd && !localStall && !core.stallIm && !core.stallDm) {
    ...
    core.regFileCopy[wbOut_temp.rd] = wbOut_temp.value;
  }
...
// void doCore(IncompleteMemory im, IncompleteMemory dm, bool globalStall)
void doCoreHLS_axi(bool globalStall,  unsigned int im[1 << 24],  unsigned int  dm[1 << 24],unsigned int * out1)
{
//#pragma HLS INTERFACE m_axi port=data  bundle=memory
#pragma HLS INTERFACE m_axi port=im  bundle=imemory
#pragma HLS INTERFACE m_axi port=dm  bundle=dmemory
  ...
  while (1) {
    doCycle(core, globalStall, im,dm,out1);
  }
}

// void doCore(IncompleteMemory im, IncompleteMemory dm, bool globalStall)
void doCoreHLS(bool globalStall,  unsigned int im[1 << 24],  unsigned int  dm[1 << 24],unsigned int * out1)
{
//#pragma HLS INTERFACE m_axi port=data  bundle=memory
//#pragma HLS INTERFACE m_axi port=im  bundle=imemory
//#pragma HLS INTERFACE m_axi port=dm  bundle=dmemory
#pragma HLS RESOURCE variable=im core=RAM_2P_URAM:
#pragma HLS RESOURCE variable=dm core=RAM_2P_URAM:
  Core core;
...
void doCoreMicroHLS(ac_int<32, false> startpc, bool globalStall, unsigned int  im[1 << 24], unsigned int dm[1 << 24],volatile unsigned int * out1)
{
 //#pragma HLS INTERFACE m_axi port=im  bundle=imemory
//#pragma HLS INTERFACE m_axi port=dm  bundle=dmemory

 //#pragma HLS INTERFACE m_axi port=im  bundle=memory num_read_outstanding=0 num_write_outstanding=0
//#pragma HLS INTERFACE m_axi port=dm  bundle=memory num_read_outstanding=0  num_write_outstanding=0


//#pragma HLS RESOURCE variable=im core=RAM_1P_BRAM
//#pragma HLS RESOURCE variable=dm core=RAM_1P_BRAM
//#pragma HLS RESOURCE variable=m_axi core=RAM_2P_URAM:
//#pragma HLS interface ap_ctrl_none port=return
//#pragma HLS RESOURCE variable=dmData core=RAM_2P_URAM:
 static Core core;
 /* IncompleteMemory<4> imInterface = IncompleteMemory<4>(imData);
  IncompleteMemory<4> dmInterface = IncompleteMemory<4>(dmData);
