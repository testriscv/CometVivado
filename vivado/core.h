...
struct Core {
  ...
  DirectMemory<4> dm ; //(mem.data());
  DirectLoadMemory<4> im ; //(mem.data());

  ac_int<32, true> regFile[32];
  ac_int<32, true> regFileCopy[32];
...
void doCycle(struct Core& core, // Core containing all values
             bool globalStall,unsigned int /*ac_int<32, false>*/ dataI [1<<24],unsigned int /*ac_int<32, false>*/ dataM [1<<24],volatile unsigned int *out1);
#endif // __CORE_H__
